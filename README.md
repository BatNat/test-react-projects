...
spyOnМетод Jest возвращает фиктивную функцию , но на данный момент мы не заменили функциональность функции выборки . Для этого нам нужно использовать .mockImplementation(callbackFn)метод и вставить то, что мы хотим заменить, fetchв качестве callbackFnаргумента. Мы можем просто использовать тот же fetchмакет, что и раньше, где мы заменили fetchна () => Promise.resolve({ json: () => Promise.resolve([]) }).

Если вы не знакомы с тестовыми шпионами и имитирующими функциями, заключается в том, что шпионская функция не меняет никаких функциональных возможностей, в то время как фиктивная функция заменяет функциональность. С помощью тестовых шпионов вы можете записывать все вызовы этой функции. Позже вы можете утверждать, какие аргументы получила шпионская функция. Например, мы могли бы утверждать, что это fetchбыло вызвано 
